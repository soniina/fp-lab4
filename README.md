...
6.  **SwitchTurn:** Передача хода следующему игроку.
...
Транслирует список переходов `unoMachine` в текстовый формат DOT (Graphviz). Это позволяет автоматически генерировать актуальную схему автомата при каждом запуске программы.

![Граф состояний Uno](graphviz.svg)
